...
# # GUI prompt to get server IP simulating DNS resolution [7]
# root = tk.Tk()
# root.withdraw()  
...
server_ip = simpledialog.askstring("Server IP", "Enter the server IP:", parent=root)
client.settimeout(None)
client.connect((server_ip, 12345))  # [3] TCP reliable connection

method = client.recv(1024).decode().strip().lower()
key = client.recv(1024)

# [4] Setup encryption based on received method
if method == "fernet":
    cipher_suite = Fernet(key)
    print(f"[FERNET] Key: {key.decode()}")
else:
        ...
        try:
            ...
            if decrypted_header.startswith("FILE:"):
                ...
                while len(encrypted_data) < filesize:
                    chunk = client.recv(min(4096, filesize - len(encrypted_data)))
                    encrypted_data += chunk
                filepath = os.path.join("received_" + filename)
                with open(filepath, "wb") as f:
                    f.write(decrypt(encrypted_data))
                print(f"[RECEIVED FILE] Saved {filepath}")
                text_area.config(state='normal')
                text_area.insert(tk.END, f"{filename} received and saved.\n")
                text_area.config(state='disabled')
                text_area.yview(tk.END)
                add_file_button(filename, filepath)
            else:
                text_area.config(state='normal')
                text_area.insert(tk.END, f"{decrypted_header}\n")
                text_area.config(state='disabled')
                text_area.yview(tk.END)
        except Exception as e:
            print(f"[ERROR] {e}")
